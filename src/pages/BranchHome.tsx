...
const branchConfig: Record<string, { name: string; icon: any; color: string; accent: string }> = {
  ...
  Civil: { name: 'Civil Engineering', icon: HardHat, color: 'bg-navy', accent: 'text-accent-orange' },
  Mechanical: { name: 'Mechanical Engineering', icon: Settings2, color: 'bg-navy', accent: 'text-accent-orange' },
  EE: { name: 'Electrical Engineering', icon: Zap, color: 'bg-navy', accent: 'text-accent-orange' },
  ECE: { name: 'Electronics & Communication', icon: Zap, color: 'bg-navy', accent: 'text-accent-orange' },
};
...
export const BranchHome: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubAchievements = subscribeToAchievements((achs) => {
      const filtered = achs.filter(a => a.branch === branchId);
  ...
  return (
    <div className="bg-[#fcfcfc] min-h-screen animate-in fade-in duration-700">
      ...
      <div className="sticky top-[72px] md:top-[88px] z-40 bg-navy/95 backdrop-blur-md border-b border-white/10 shadow-lg md:block">
        <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
           <div className="flex items-center gap-3">
             <config.icon className={cn("w-6 h-6", config.accent)} />
             <div className="h-4 w-[1px] bg-white/20"></div>
             <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">{config.name}</p>
           </div>
      ...
      {/* Hero Section */}
      <section id="home" className="relative py-24 md:py-36 px-6 overflow-hidden flex flex-col items-center justify-center text-center text-white bg-navy">
        ...
        >
          <Badge className="bg-white/10 backdrop-blur-md text-white/80 border-white/20 uppercase tracking-[0.2em] font-black text-[9px] px-5 py-1.5">
            CAREER PORTAL FROM GEC VAISHALI
          </Badge>
          ...
          <p className="text-white/60 text-sm md:text-base font-medium max-w-xl mx-auto leading-relaxed italic uppercase tracking-wider">
            Empowering the engineers of {branchId} by bridging the gap between academic excellence and industry standards.
          </p>
        </motion.div>
      </section>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            ...
            <div className="bg-white p-10 md:p-14 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100 relative group overflow-hidden">
               ...
               <p className="text-slate-500 text-base md:text-lg leading-relaxed italic font-medium">
                 "Welcome to the {branchId} department's career gateway. Our curriculum is designed to push the boundaries of traditional learning, integrating modern engineering practices with core theoretical foundations."
               </p>
            </div>
          </motion.div>
          <motion.div 
          ...
          >
            <div className="relative inline-block group">
              ...
              <img 
                src="https://picsum.photos/seed/hod-branch/600/600" 
                alt="HOD Sir" 
            ...
            <div className="space-y-1 relative z-10">
              ...
              <p className="text-[10px] font-black text-accent-orange uppercase tracking-[0.3em]">HOD, {branchId} Engineering</p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <div className="relative inline-block group">
              <div className="absolute -inset-4 bg-accent-orange/10 rounded-[3rem] blur-xl opacity-0 group-hover:opacity-100 transition-opacity"></div>
              <img 
                src="https://picsum.photos/seed/tpo-branch/600/600" 
                alt="TPO Head" 
                className="w-64 h-80 md:w-80 md:h-[28rem] object-cover rounded-[2.5rem] shadow-2xl relative z-10 border-4 border-white"
                referrerPolicy="no-referrer"
              />
            ...
            <div className="space-y-1 relative z-10">
              <h3 className="text-2xl font-black uppercase text-navy italic">DR. SHIVANGI SAXENA</h3>
              <p className="text-[10px] font-black text-accent-orange uppercase tracking-[0.3em]">TPO HEAD, {branchId} Engineering</p>
            </div>
          </motion.div>
          <motion.div 
            initial={{ opacity: 0, x: 30 }}
          ...
          >
            ...
            <div className="bg-white p-10 md:p-14 rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] border border-slate-100 relative group overflow-hidden">
               ...
               <p className="text-slate-500 text-[14px] md:text-[16px] leading-relaxed italic font-medium">
                 "It gives us immense pleasure to extend to you a cordial invitation to participate in the campus placement process of GEC Vaishali. With a state-of-the-art infrastructure, highly dedicated team of faculty and with well equipped laboratory setup, we provide a highly conductive environment for teaching and learning process."
               </p>
            </div>
          </motion.div>
        </div>
                ...
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 20 }}>
                  ...
                  <Bar dataKey="value" radius={[12, 12, 12, 12]} barSize={80}>
                    {chartData.map((entry, index) => (
                      <Cell key={index} fill={index % 2 === 0 ? '#4ade80' : '#22c55e'} />
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
